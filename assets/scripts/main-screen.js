$(document).ready(function () {
    ...
    function load_js(id){
        ...
        $(id+".dropdown-menu li").click(function(){      // ???????, ??????????? ???????? ? ????
            ...
            obj['quote']['Ventilationstyp'] = $(this).attr('data-value');
        ...
        $(id+'button.boxButtonService').click(function(){
            if ($(this).siblings('.orange').length > 0){
                alertModal ($(this),true);
                    ...
                    var link = 'mailto:info@besiktigaste.se?subject=SERVICE PROVIDERS: from ' +
  ...
  //TW: den här funktionen samlar in form-data. 
    function request(e){
        var shortcut = '';
			...
			//sätts i rad 87 ovan
			Ventilationstyp = $('div#'+e).attr('Ventilationstyp'),
            affiliateId = '913',
        ...
        for (var key in obj['quote']){
            switch (key) {
				...
				case 'noApartments':
					...
					break;
				case 'Ventilationstyp':
					Ventilationstyp = obj['quote'][key];
					break;           
                default:
		...
		var resultXml = '<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>'+
			...
			//(Ventilationstyp ? '<Ventilationstyp>' + Ventilationstyp + '</Ventilationstyp>':'<Ventilationstyp/>') +

$(document).ready(function () {
    ...
    function load_js(id){
        ...
        // ???????
		//TW: här sätts ett värde på ventilationstypen - vi är i dropdown for ventilationstyp
        ...
        $(id+".dropdown-menu li").click(function(){      // ???????, ??????????? ???????? ? ????
            ...
            var tabActive = $(this).parents('.tab-pane.active').attr('id'),
                dataId = $(this).parents('.btn-group').find('.dropdown-toggle').attr('data-id');
            
			//TW: här sätts ett värde på ventilationstypen 
            obj['quote']['ventilationType'] = $(this).attr('data-value');
            
			var next = $(this).parents('.btn-group.dropdownBoil.orange').removeClass('orange').next();
            if (next.prop("tagName") && next.prop("tagName").toLowerCase() == 'img'){
                next.remove();
            }
            console.log(obj);
        });
        ...
        $(id+'button.boxButtonService').click(function(){
            if ($(this).siblings('.orange').length > 0){
                alertModal ($(this),true);
                    ...
                    var link = 'mailto:ny.kund@besiktigaste.se?subject=SERVICE PROVIDERS: from ' +
    ...
    function okModal(){
        $('body').prepend('<div class="modal_form_success"><img src="/assets/images/cross.png" class="modal_close_success"><img src="/assets/images/ok.png" class="alertImg"><span class="alertTitle">Tack f&ouml;r ditt meddelande!</span><span class="alertText">Vi kontaktar dig inom kort.</span></div><div class="overlay"></div>');
  ...
  //TW: den här funktionen samlar in form-data. 
    function request(e){
        var shortcut = '';
            locationId = '',
            locationShortCut = '',
            ...
            userEmail = '',
            additionalInfo = '',
            Fastighetsbeteckning = $('div#'+e).attr('Fastighetsbeteckning'),
            Fastighetsagare = $('div#'+e).attr('Fastighetsagare'),
			noApartments = $('div#'+e).attr('noApartments'),
			Ventilationstyp = $('div#'+e).attr('ventilationType'),
            affiliateId = '913',
		...
		//TW: alla fields i webform ska definieras här 
        for (var key in obj['quote']){
            switch (key) {
				...
				case 'Fastighetsagare':
					Fastighetsagare = obj['quote'][key];
					break;     
				case 'Fastighetsbeteckning':
					Fastighetsbeteckning = obj['quote'][key];
					break;    
				case 'Fastighetsadress':
					Fastighetsadress = obj['quote'][key];
					break;   
				case 'Fastighetsort':
					Fastighetsort = obj['quote'][key];
					break;   
				case 'noApartments':
					noApartments = obj['quote'][key];
					break;
				case 'ventilationType':
					ventilationsType = obj['quote'][key];
					break;           
                default:
		...
		//TW: den här funktionen preparerar för utskrift i mailer.php 
		var resultXml = '<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>'+
            ...
            (formURL ? '<formURL>' + formURL.value + '</formURL>':'<formURL/>') +
			...
			(Fastighetsagare ? '<Fastighetsägare>' + Fastighetsagare + '</Fastighetsägare>':'<Fastighetsägare/>') +
			(Fastighetsbeteckning ? '<Fastighetsbeteckning>' + Fastighetsbeteckning + '</Fastighetsbeteckning>':'<Fastighetsbeteckning/>') +
			(Fastighetsadress ? '<Fastighetsadress>' + Fastighetsadress + '</Fastighetsadress>':'<Fastighetsadress/>') +
			(Fastighetsort ? '<Fastighetsort>' + Fastighetsort + '</Fastighetsort>':'<Fastighetsort/>') +
			(noApartments ? '<Antal-lägenheter>' + noApartments + '</Antal-lägenheter>':'<Antal-lägenheter/>') +
			//(332358 ? '<Ventilationstyp>' + Ventilationstyp + '</Ventilationstyp>':'<Ventilationstyp/>') +
...
/*        if (!addJs){
        ...
        var hostname = a.hostname,
            //baseUrl = 'https://' + hostname + '/assets/scripts/';
